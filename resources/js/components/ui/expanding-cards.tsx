...
>(({ className, items, defaultActiveIndex = 0, ...props }, ref) => {
    ...
    const gridStyle = React.useMemo(() => {
        ...
        if (isDesktop) {
            const columns = items
                .map((_, index) => (index === activeIndex ? "3fr" : "1fr"))
                .join(" ");
        ...
        >
            {items.map((item, index) => (
                <li
                    key={item.id}
                    className={cn(
                        "group relative cursor-pointer overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm",
                        "md:min-w-[80px]",
                        "min-h-0 min-w-0",
                    )}
                ...
                >
                    ...
                    <article className="absolute inset-0 flex flex-col justify-end gap-1 p-4 md:p-5">
                        ...
                        <h3 className="text-lg md:text-xl font-bold text-white opacity-0 transition-all duration-300 delay-150 ease-out group-data-[active=true]:opacity-100">
                            {item.title}
                        </h3>
                        <p className="w-full text-sm text-white/80 opacity-0 transition-all duration-300 delay-200 ease-out group-data-[active=true]:opacity-100 line-clamp-4 md:line-clamp-6">
                            {item.description}
                        </p>
                    </article>
                </li>
